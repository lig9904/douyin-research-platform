...
- 项目、CI 与 Windmill Python 脚本统一到 CPython 3.14，开发环境固定 3.14.7。
- 研究台新增“今日研判”：先展示规则排序的合并事实、运行结果和费用口径，再按视频 ID
  下钻指标来源、历史快照、评论样本、转写与已审核 L3。
- 本机只读 MCP 扩展为五项，Windmill Gateway 扩展为十项；新增每日研判、研究任务和
  实账/估算分离的费用查询，并允许返回公开标题与账号昵称。

### Verification boundary

- Python 3.14 本机回归和前端打包通过；测试服务器 Worker、研究台和 MCP Gateway 仍需
  独立部署及现场烟测，不能把代码通过写成服务器已生效。

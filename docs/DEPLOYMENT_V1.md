...
前期建议：
- default worker: 本地单任务环境 1 个执行槽；测试服 2 个独立容器执行槽，约 1–2 vCPU / 2–4GB RAM，供采集与页面查询并行使用。黄金采集在 App 任务内直接执行，不再同步等待子任务；业务并发由数据库锁控制。两个副本共享的 `/tmp/windmill/cache` 是可丢弃的 Worker 缓存，首次创建命名卷时禁用镜像目录复制，避免双副本初始化竞争；业务数据、脚本配置和数据库不在此卷中。
...
- script scope 使用一个 `mcp:scripts:` 条目逐项列出十个
  `f/content_research/research_tools/<tool>` 完整路径，不使用目录通配符
- 当前仅暴露十项只读 research scripts，以及 Windmill 自动提供、受同一路径 scope
...
脱敏 Authorization header。测试服务器已完成该设置及以下验收；新环境上线仍需以
自身 token 重做。`tools/list` 仅含十项业务脚本和

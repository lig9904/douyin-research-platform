...
回滚：先确定需要回退的是应用内容、Worker/服务镜像，还是数据库；不得把“恢复最后备份”当成默认操作。只停用受影响的 schedule/runnable，记录原启用状态并等待相关运行任务结束。应用回退保留当前数据库，按 [应用回滚步骤](APPLICATION_ROLLBACK_V1.md) 核对前后端及 schema 兼容性。数据库备份仅恢复到新建隔离库；切换运行数据库属于另一次明确变更，必须先评估备份后新增用户数据、任务和账本并取得确认，禁止覆盖原库。恢复失败保留原运行库与失败证据，不反复覆盖源库。

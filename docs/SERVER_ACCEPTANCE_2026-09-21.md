...
首个检查点日期：2026-09-21；下方按时间倒序追加。范围为浏览器 JumpServer、真实测试服务器数据库与研究台页面，不是用户最终 V1 业务验收。

## 渔岛景区实际使用试跑（2026-09-23 UTC 06:11 后）

- 从研究台激活一次性“渔岛”关键词任务并由 `dispatch_due_research_briefs` 实际派发：调度运行成功，子流程的研究运行 `8bc1176e-91f2-4d39-b5ad-f2d946034fd3` 失败。账本显示搜索一次成功（本地报价 USD 0.010）、详情一次 HTTP 400；无 SDK 重试、无 LLM 调用。失败原因尚未定位，不以本地费用分类替代供应商最终日账。
- 5 条搜索候选均偏向《梦幻西游》“渔岛战队/武神坛”，与秦皇岛渔岛景区业务不相干。候选已进入发现/视频资产，但不得用作景区热点、借鉴建议或业务价值验收依据。
- 研究台复查显示 4 个任务、7 条 success 和 1 条 failed；该一次性任务已暂停、“下次 —”、最近状态为 failed。周期低粉榜任务仍 active，下一次北京时间 17:05。失败记录和账本保留供定位；不盲目重复付费请求。
- 因此此前技术路径通过不等于渔岛业务验收通过。正式 V1 发布新增门槛：关键词相关性门控、详情失败定位与复验，以及至少一次相关样本的选题—执行—经营结果复盘；详见 [渔岛业务使用验收与缺口](YUDAO_BUSINESS_ACCEPTANCE_V1.md)。

## 研究任务调度切换与费用显示（2026-09-23 UTC 06:00 后）

- GitHub PR #136 经独立复审、provider-tests 与 web-dashboard CI 通过后合并；测试服务器从 clean 的 `6e309213` 固定到 `36ee4cf4f5022cfe0b261ae8ac079cc1463d10d4`。发布前备份 `/srv/douyin-research-test/backups/20260923T060135Z` 经独立校验 `BACKUP_VALID`，包含双库和 globals；未进行数据库迁移或 Secret 变更。
- 迁移 020 已在服务器台账中；`research_brief` 有 3 个任务、`research_brief_run` 有 7 条 success。活动的 6 小时低粉榜元数据任务已于 UTC 03:05 到点派发，下一次为 UTC 09:05；关键词和一次性低粉榜任务均暂停。每小时 `dispatch_due_research_briefs` 保持启用，旧 `scheduled_research_cycle` 已在 Windmill 界面停用，ASR 轮询和费用同步仍启用。旧固定计划停用前产生的运行作为历史保留，不误算为新 dispatcher 结果。
- 只发布 `f/content_research/research_dashboard` Raw App，CLI lint 19 个 runnable 与前端构建通过，专用 push 退出 0；发布日志 `/srv/douyin-research-test/evidence/raw-app-cost-display-36ee4cf.log`。活动版本从 38 升到 39，数据库确认版本 39 包含新增后端费用字段和前端展示逻辑。浏览器重新加载后，任务费用分组的 CNY/unknown 2 项和两条 ASR 任务明细均显示“金额未知 / 待对账”；真实已知 L3 估算金额仍单独显示。SQL/前端测试还覆盖“部分已知金额 + 其余待对账”与真实 0 的区分，服务器未人为制造收费测试记录。
- 同页 TikHub 供应商 2026-09-22 账期为 USD 0.057、请求 30 次、其中付费 8 次，America/Los_Angeles、未终账，最后同步显示北京时间 2026-09-23 14:00；报价估算与实际日账分栏。此金额是当时页面快照，不是未来账期的固定值。测试服务器五容器继续运行；应用服务器未新增 Nginx。
- 一条历史 `media_ingestion` 在北京时间 2026-09-22 20:10 以 `media_download_failed` 结束，`external_paid_calls=0`；未把失败伪装成功或做付费重试。它是旧固定链路的失败记录，仍需作为运行异常跟踪，但不覆盖此前内容充分样本的 ASR→L3 验收，也不代表新元数据周期失败。

发布边界：上述证明代码、调度和费用 UI 已在测试服务器生效，不代表用户已认可“建任务→看合并结果→看原始依据→看分析→看日费用”的业务体验。正式标签仍待用户确认；火山实际日实扣自动读取、异机灾备、三账号成员 ACL 与外部告警闭环不在这次验证中。
...
- UTC 12:10（北京时间 20:10）的计划任务 `01a0c310-376c-478a-170d-0d71f402e2c5` 按时成功；发现阶段 1 次真实 Provider 调用、2 条观察、2 条评分，new_candidate_count=0，所以下游评论和媒体步骤按门控跳过。下一任务 `01a0c3df-81d7-152a-ed12-868b65da8305` 已生成，计划于北京时间 9 月 22 日 02:10 执行。
- 《那一刻，我也被净化了》约 73.1 秒音频经用户试听审核后完成真实火山 ASR：143 个非空白字符、126 个中文字符、8 个分段、quality_status=usable。ASR 供应商实扣金额仍为 unknown/NULL，没有写成免费。
- 用户核对并确认指纹 `77d1ed4a7215ba26787fafa199f11fd135ce3f8eac05e7cc96dc216a80ae4182` 的 L3 正文后，方舟任务 `3f42ea22-ef31-4e32-a6fb-ca803318ee92` completed、attempt_count=1、sdk_retries=0；模型 `doubao-seed-2-0-lite-260428`，估算 LLM 费用 0.005883 CNY。立即重放 examined=2、existing=2、dispatched=0。
- 页面显示视频 L3、完整转写和七类结构化结果；评论、对比、机制、IP 和视觉缺失均被明确标注，没有用数字统计冒充原评论语义。非 Secret 的预算预览变量已绑定实际模型/版本/Prompt/价格版本，仍诚实显示 `estimate_required`，没有虚构通用单任务报价。

至此，数据库隔离恢复、ASR 受控进程失败恢复、MinIO 隔离对象丢失恢复、Raw App 双向回滚、新版计划首个真实到点周期和内容充分业务案例均有现场证据。剩余动作是用户完成最终业务使用验收后发布正式 V1；火山实际日账自动同步和 ASR 实扣金额仍是后续增强，不能用任务级估算替代。

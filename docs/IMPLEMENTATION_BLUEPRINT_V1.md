...
2026-09-20计价修订：调用优先级和费用以 [业务接口调用策略](API_BUSINESS_CALL_POLICY_V1.md)
为准；本文件是目标蓝图，不能据此认定自动调度、限速或对账已经上线。
...
候选先按字段新鲜度筛选和去重。完整详情比较单条、10批、50批及折扣后的总成本；
增长监测优先验证统计接口（2条/USD0.001、50条/USD0.025），不重复拉完整内容。
50条详情USD0.050，仅满批或折扣达到合适填充率时选择；尾批允许单条。

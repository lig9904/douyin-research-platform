...
1. 调用方明确传入 `privacy_reviewed=true` 和非空、格式受限的隐私复核版本；该检查在连接数据库之前完成。
2. 视频存在且 `research_level >= 2`。
3. 视频已经被 L2→L3 闸门以 `selected` 结果选中。
4. 存在评论特征快照。
5. 存在转写，并且最新转写状态为 `usable`、`low_confidence` 或 `no_speech`。
...
评论 snapshot 的 `metadata` 不会整体透传；组装器只输出固定的范围声明。转写正文和视频标题/描述仍可能含个人信息，所以 `privacy_reviewed=true` 是调用方对已完成内容复核的事实声明，不是组装器自动识别个人信息的声明。
...
正式执行的 `evidence_factory` 必须返回 `L3EvidenceBundle`。执行控制层会在构造 Provider 和预占预算之前核对证据视频 ID 与请求视频 ID、证据指纹与请求指纹；普通 Mapping 不能绕过该绑定。

正式付费执行仍由 L3 执行控制层负责预算、确认、单并发、零重试和故障对账；证据组装器不扩大任何执行授权。

## 验收边界

- 单元/集成测试证明：准入失败关闭、选择最新快照、NULL/0 保真、指纹稳定、敏感标识不透传、超限拒绝和数据库零写入。

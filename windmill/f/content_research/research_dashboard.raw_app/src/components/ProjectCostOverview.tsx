...
export default function ProjectCostOverview({ projectId }: { projectId: string }) {
  ...
  return <div className="operations-page">
    <Alert type="info" showIcon message="项目执行账本" description="按项目、上海自然日和币种展示项目发现、评论、账号核验、ASR 与 L3 任务流水；发现、评论和账号核验费用按任务启动日归属，跨午夜时可能不同于供应商实扣日。未与供应商分笔实扣对齐的费用仍是估算，币种不折算，未知费用不计为零。失败运行的币种未确定时单列为 UNKNOWN。" />
        ...
        <Table size="small" rowKey={(row) => `${row.cost_date}-${row.cost_currency}`} pagination={false}
          dataSource={report.records} locale={{ emptyText: '所选日期内尚无项目发现、评论、账号核验、ASR 或 L3 费用记录' }} columns={[
            ...
            { title: '待对账', render: (_, row) => row.unknown_task_count
              ? <><Tag color="warning">{row.unknown_task_count} 项金额或执行状态未闭合</Tag>{row.unbilled_job_count > 0 && <Tag color="error">含 {row.unbilled_job_count} 项执行中断/未落账</Tag>}</>

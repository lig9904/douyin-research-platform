...
_WORKER_PATH = "f/content_research/analysis/run_project_reviewed_l3"
_BATCH_SIZE = 12
...
def _pending(dsn):
    with psycopg.connect(dsn) as conn:
        ...
        return conn.execute(
            """select review.id
               from project_l3_privacy_review review
               join project_transcript transcript
                 on transcript.id=review.transcript_id and transcript.project_id=review.project_id
                 and transcript.video_id=review.video_id
               join project_asr_media_review media_review
                 on media_review.id=transcript.media_review_id
                 and media_review.project_id=review.project_id
                 and media_review.video_id=review.video_id
               ...
               where review.status='approved' and media_review.status='approved'
                 ...
                 and not exists (
                   select 1 from project_l3_execution_job job where job.privacy_review_id=review.id
                 )
...
def main() -> dict:
    ...
    try:
        ...
        with scheduler_slot(dsn) as acquired:
            ...
            rows = _pending(dsn)
            completed = needs_attention = 0
            for (review_id,) in rows:
                result = wmill.run_script(path=_WORKER_PATH, args={"review_id": str(review_id)},
        ...
        raise RuntimeError("project L3 reviewed dispatch unavailable") from None
    if needs_attention:
        raise RuntimeError(f"project L3 reviewed dispatch requires attention: {needs_attention}")
    return {"selected": len(rows), "completed": completed, "needs_attention": 0}
